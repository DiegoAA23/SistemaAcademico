<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 mt-6 mb-2 dark:text-gray-200 leading-tight">
            {{ __('Cursos') }}
        </h2>
    </x-slot>
                ...
                <div class="mb-4">
                    ...
                    <table class="table-auto w-full">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="border px-4 py-2 text-gray-900 dark:text-white text-center">{{ $clase->	id_curso }}</td>
                                <td class="border px-4 py-2 text-gray-900 dark:text-white text-center">{{ $clase->nombre_clase }}</td>
                                <td class="border px-4 py-2 text-gray-900 dark:text-white text-center">{{ $clase->profesor->nombre . ' ' . $clase->profesor->apellido }}</td>
                                <td class="border px-4 py-2 text-gray-900 dark:text-white text-center">{{ $clase->periodo }}</td>
                                <td class="border px-4 py-2 text-gray-900 dark:text-white text-center">{{ $clase->estado->descripcion }}</td>

                                <td class="border px-4 py-2 text-center">
                                    <div class="flex justify-center">
                                        <a href="{{ route('claseEdit', $clase->id_curso) }}" class="bg-cyan-900 dark:bg-cyan-900 hover:bg-indigo-600 dark:hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded mr-2">Editar</a>
                                        <button type="button" class="bg-gray-700 dark:bg-gray-700 hover:bg-red-600 dark:hover:bg-red-900 text-white font-bold py-2 px-4 rounded" onclick="confirmDelete('{{ $clase->id_curso }}')">Inhabilitar</button>
                                    </div> 
                                </td>
                            </tr>
                            @endforeach
